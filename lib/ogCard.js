...
export async function ogFonts() {
  // Full faces rather than a text subset: the case study cards render project
  // names and taglines that aren't known here.
...
/*
  One card, two shapes. Defaults render the site card; the case study routes
  pass their own eyebrow/title/subtitle so a shared link previews the project
...
export function OgCard({
  ...
  titleTop = "BAGAS ADY",
  titleBottom = "SANTOSO",
  subtitle = `${site.role} · Community · Research`,
  command = "bagas@web3 ~ $ ./profile-card",
  footLeft = `${site.yearsInWeb3} YRS IN WEB3 · ${site.location.toUpperCase()}`,
} = {}) {
  const mono = "DM Mono";
  const display = "Space Grotesk";
  // a long project name needs to step down a size to stay on one line
  const longestTitleLine = Math.max(titleTop.length, (titleBottom || "").length);
  const titleSize = longestTitleLine > 14 ? 68 : longestTitleLine > 10 ? 86 : 104;
